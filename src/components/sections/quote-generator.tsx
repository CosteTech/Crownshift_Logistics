               ...
               <Card className="w-full max-w-md shadow-2xl animate-in fade-in-50 zoom-in-95">
                ...
                <CardFooter className="bg-secondary p-6 rounded-b-lg">
                  <div className="w-full">
                    <h4 className="font-semibold font-headline mb-3">Quote Breakdown:</h4>
                    {state.breakdown && (
                      <div className="space-y-1 text-sm">
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Base Rate:</span>
                          <span>${state.breakdown.baseRate.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Distance Charge:</span>
                          <span>${state.breakdown.distanceCharge.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Volume Surcharge:</span>
                          <span>${state.breakdown.volumetricSurcharge.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Weight Surcharge:</span>
                          <span>${state.breakdown.weightSurcharge.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Handling Fee:</span>
                          <span>${state.breakdown.handlingFee.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Urgency Surcharge:</span>
                          <span>${state.breakdown.urgencySurcharge.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Insurance:</span>
                          <span>${state.breakdown.insuranceFee.toFixed(2)}</span>
                        </div>
                      </div>
                    )}
                    {state.estimatedDeliveryDays && (
                      <p className="mt-3 text-sm text-muted-foreground">
                        Estimated Delivery: {state.estimatedDeliveryDays} day{state.estimatedDeliveryDays > 1 ? 's' : ''}
                      </p>
                    )}
                    {state.summary && (
                      <p className="mt-2 text-sm text-muted-foreground">{state.summary}</p>
                    )}
                    {state.recommendations && (
                      <p className="mt-2 text-sm text-primary">{state.recommendations}</p>
                    )}
                  </div>
                </CardFooter>
              </Card>
